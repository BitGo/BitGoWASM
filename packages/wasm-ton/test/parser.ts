import * as assert from "assert";
import { Transaction } from "../js/transaction.js";
import { parseTransaction } from "../js/parser.js";
import type { ParsedTransaction } from "../js/parser.js";

// BitGoJS fixtures
const signedSendTx =
  "te6cckEBAgEAqQAB4YgBJAxo7vqHF++LJ4bC/kJ8A1uVRskrKlrKJZ8rIB0tF+gCadlSX+hPo2mmhZyi0p3zTVUYVRkcmrCm97cSUFSa2vzvCArM3APg+ww92r3IcklNjnzfKOgysJVQXiCvj9SAaU1NGLsotvRwAAAAMAAcAQBmQgAaRefBOjTi/hwqDjv+7I6nGj9WEAe3ls/rFuBEQvggr5zEtAAAAAAAAAAAAAAAAAAAdfZO7w==";

const signedNominatorTx =
  "";

const whalesDepositTx =
  "";

const whalesWithdrawalTx =
  "";

const tokenSendTx =
  "";

describe("parseTransaction", () => {
  it("should parse a send transaction", () => {
    const tx = Transaction.fromBoc(signedSendTx);
    const parsed: ParsedTransaction = parseTransaction(tx);

    assert.strictEqual(parsed.type, "Send");
    assert.strictEqual(parsed.seqno, 6);
    assert.strictEqual(parsed.walletId, 698983191);
    assert.strictEqual(parsed.bounceable, false);
    // Amount should be a bigint
    assert.strictEqual(typeof parsed.amount, "bigint");
    assert.ok(parsed.amount > 0n);
    // Recipient should be a TON address
    assert.ok(parsed.recipient.length > 0);
    // Sender should be a TON address
    assert.ok(parsed.sender.length > 0);
    // Signature should be a hex string
    assert.ok(parsed.signature.length > 0);
  });

  it("should parse a single nominator withdraw", () => {
    const tx = Transaction.fromBoc(signedNominatorTx);
    const parsed = parseTransaction(tx);

    assert.strictEqual(parsed.type, "SingleNominatorWithdraw");
    assert.strictEqual(parsed.seqno, 0);
    // Should have public key from StateInit
    assert.ok(parsed.publicKey);
    assert.strictEqual(
      parsed.publicKey,
      "c0c3b9dc09932121ee351b2448c50a3ae2571b12951245c85f3bd95d5e7a06f8",
    );
  });

  it("should parse a whales deposit", () => {
    const tx = Transaction.fromBoc(whalesDepositTx);
    const parsed = parseTransaction(tx);

    assert.strictEqual(parsed.type, "TonWhalesDeposit");
    assert.strictEqual(parsed.seqno, 92);
    assert.strictEqual(parsed.bounceable, true);
  });

  it("should parse a whales withdrawal", () => {
    const tx = Transaction.fromBoc(whalesWithdrawalTx);
    const parsed = parseTransaction(tx);

    assert.strictEqual(parsed.type, "TonWhalesWithdrawal");
    assert.strictEqual(parsed.seqno, 93);
  });

  it("should parse a token send", () => {
    const tx = Transaction.fromBoc(tokenSendTx);
    const parsed = parseTransaction(tx);

    assert.strictEqual(parsed.type, "SendToken");
    assert.strictEqual(parsed.seqno, 0);
    // Should have token amount as bigint
    assert.ok(parsed.tokenAmount !== undefined);
    assert.strictEqual(typeof parsed.tokenAmount, "bigint");
    assert.strictEqual(parsed.tokenAmount, 1000000000n);
    // Should have token recipient
    assert.ok(parsed.tokenRecipient !== undefined);
    assert.ok(parsed.tokenRecipient!.length > 0);
  });

  it("should return amounts as bigint", () => {
    const tx = Transaction.fromBoc(signedSendTx);
    const parsed = parseTransaction(tx);

    // amount must be bigint
    assert.strictEqual(typeof parsed.amount, "bigint");
    // expireTime must be bigint
    assert.strictEqual(typeof parsed.expireTime, "bigint");
  });
});
