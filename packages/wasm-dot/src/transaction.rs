...
///
/// Iterates `metadata.extrinsic().signed_extensions()` and for each:
/// - Known extensions (era, nonce, tip): decode and extract the value
/// - Unknown extensions: skip bytes using scale_decode with IgnoreVisitor
fn parse_signed_extensions_from_metadata(
    ...
    for ext in extensions {
        ...
        // Check if this extension encodes zero bytes (type is () / empty tuple).
        // Most extensions like CheckNonZeroSender, CheckSpecVersion, CheckWeight
        // have extra_ty = () and add no bytes to the extrinsic body.
        ...
        match id {
            "CheckMortality" | "CheckEra" => {
                let (decoded_era, era_size) = decode_era_bytes(&bytes[cursor..])?;
                era = decoded_era;
                cursor += era_size;
            }
            "CheckNonce" => {
                let mut input = &bytes[cursor..];
                let decoded = <Compact<u32>>::decode(&mut input).map_err(|e| {
                    WasmDotError::InvalidTransaction(format!("Invalid nonce: {}", e))
                })?;
                nonce = decoded.0;
                cursor = bytes.len() - input.len();
            }
            ...
            _ => {
                // Unknown extension — skip bytes using the type registry
                let consumed = skip_type_bytes(&bytes[cursor..], ty_id, metadata)?;
                cursor += consumed;
            }
        }
...
/// Extensions with `extra_ty = ()` encode zero bytes in the extrinsic.
...
/// Skip bytes for a given type using scale_decode's IgnoreVisitor.
/// Returns the number of bytes consumed.
fn skip_type_bytes(bytes: &[u8], ty_id: u32, metadata: &Metadata) -> Result<usize, WasmDotError> {
    ...
    visitor::decode_with_visitor(
        ...
        ty_id,
        metadata.types(),
        visitor::IgnoreVisitor::new(),
    )
    .map_err(|e| {
        WasmDotError::InvalidTransaction(format!(
            "Failed to skip signed extension type {}: {}",
            ty_id, e
        ))
    })?;
