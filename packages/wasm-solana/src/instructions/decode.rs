...
fn decode_system_instruction(ctx: InstructionContext) -> ParsedInstruction {
    ...
    match instr {
        ...
        SystemInstruction::InitializeNonceAccount(authority) => {
            // This is part of CreateNonceAccount flow, but we expose it separately
            if ctx.accounts.len() >= 1 {
                ParsedInstruction::CreateNonceAccount(CreateNonceAccountParams {
                    from_address: String::new(), // Not available in this instruction alone
                    nonce_address: ctx.accounts[0].clone(),
                    auth_address: authority.to_string(),
                    amount: String::new(), // Not available in this instruction alone
                })
            } else {
                make_unknown(ctx)
            }
...
fn decode_stake_instruction(ctx: InstructionContext) -> ParsedInstruction {
    ...
    match instr {
        ...
        StakeInstruction::Initialize(authorized, _lockup) => {
            // This is part of StakingActivate flow
            // Accounts: [0] stake, [1] rent_sysvar
            if ctx.accounts.len() >= 1 {
                ParsedInstruction::StakingActivate(StakingActivateParams {
                    from_address: authorized.staker.to_string(),
                    staking_address: ctx.accounts[0].clone(),
                    amount: String::new(),    // Amount comes from CreateAccount
                    validator: String::new(), // Validator comes from Delegate
                })
            } else {
                make_unknown(ctx)
            }
...
// =============================================================================
// Stake Pool (Jito) Decoding
// =============================================================================

/// Instruction discriminators for SPL Stake Pool program.
const STAKE_POOL_DEPOSIT_SOL: u8 = 14;
const STAKE_POOL_WITHDRAW_STAKE: u8 = 10;

fn decode_stake_pool_instruction(ctx: InstructionContext) -> ParsedInstruction {
    // Stake pool instruction format: first byte is discriminator, followed by u64 amount
    if ctx.data.is_empty() {
        return make_unknown(ctx);
    }
    ...
    match discriminator {
        STAKE_POOL_DEPOSIT_SOL => {
            ...
            // Data: [0] discriminator (14), [1..9] lamports (u64 LE)
            // Accounts:
            //   [0] stakePool
            //   [1] withdrawAuthority
            //   [2] reserveStake
            ...
            //   [8] systemProgram
            //   [9] tokenProgram
            //   [10] optional depositAuthority (signer)
            if ctx.accounts.len() >= 8 && ctx.data.len() >= 9 {
                let lamports = u64::from_le_bytes(ctx.data[1..9].try_into().unwrap_or([0; 8]));
        ...
        STAKE_POOL_WITHDRAW_STAKE => {
            // WithdrawStake: withdraw stake from pool by burning pool tokens
            // Data: [0] discriminator (10), [1..9] poolTokens (u64 LE)
            // Accounts:
            //   [0] stakePool
            //   [1] validatorList
            //   [2] withdrawAuthority
            ...
            //   [11] tokenProgram
            //   [12] stakeProgram
            if ctx.accounts.len() >= 10 && ctx.data.len() >= 9 {
                let pool_tokens = u64::from_le_bytes(ctx.data[1..9].try_into().unwrap_or([0; 8]));
