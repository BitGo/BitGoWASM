...
import { parseTransaction, type ParsedTransaction, type InstructionParams } from "@bitgo/wasm-solana";
...
class SolanaTransactionParser extends BaseComponent {
  ...
  private parse(txData: string): void {
    ...
    if (!errorEl || !txInfoEl || !resultsEl) return;
    ...
    try {
      ...
      resultsEl.replaceChildren(
        h("div", { class: "empty-state" }, h("p", {}, "Enter a valid Solana transaction to see parsed data")),
      );
    }
  }
    ...
    if (parsed.durableNonce) {
      children.push(
        ...
        h(
          "span",
          { class: "tx-info-value" },
          h("span", { class: "durable-nonce-badge" }, "Yes"),
        ),
  ...
  private renderInstruction(instr: InstructionParams, index: number): HTMLElement {
    ...
    return h(
      ...
      h(
        ...
        h("span", { class: "instruction-type", style: `color: ${typeColor}` }, formatInstructionType(type)),
      ),
      params.length > 0
        ? h(
            "div",
