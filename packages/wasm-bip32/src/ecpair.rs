...
impl WasmECPair {
    ...
    #[wasm_bindgen]
    pub fn sign_message(&self, message: &str) -> Result<String, WasmBip32Error> {
        let signing_key = self
            .key
            .signing_key()
            .ok_or_else(|| WasmBip32Error::new("Cannot sign with public key only"))?;

        message::sign_bitcoin_message(signing_key, message)
    }

    /// Verify a Bitcoin message signature (BIP-137)
    /// Signature should be base64-encoded
    #[wasm_bindgen]
    pub fn verify_message(&self, message: &str, signature: &str) -> Result<bool, WasmBip32Error> {
        let verifying_key = self.key.verifying_key();
