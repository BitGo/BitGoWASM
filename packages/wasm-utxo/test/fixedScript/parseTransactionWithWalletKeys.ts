...
describe("parseTransactionWithWalletKeys", function () {
      ...
      .forEach((txFormat) => {
        describe(`${networkName} ${txFormat}`, function () {
          ...
          before(async function () {
            fixture = await loadPsbtFixture(networkName, "fullsigned", txFormat);
          ...
          it("should have valid unsigned transaction ID format", function () {
            const unsignedTxId = bitgoPsbt.unsignedTxId();
            assert.strictEqual(unsignedTxId.length, 64, "unsignedTxId should be 64 chars");
          ...
          it("should parse transaction and identify internal/external outputs", function () {
            ...
            parsed.inputs.forEach((input, i) => {
              ...
              assert.ok(
                typeof input.scriptType === "string",
                `Input ${i} scriptType should be string`,
              );
            });
            ...
            const opReturnData = Buffer.from("setec astronomy");
            const expectedOpReturn = Buffer.concat([
            ...
            parsed.outputs.forEach((output, i) => {
              assert.ok(
                ...
                `Output ${i} script should be Uint8Array`,
              );
              ...
              if (output.address === null) {
                ...
                assert.ok(
                  ...
                  `Output ${i} script should start with OP_RETURN`,
                );
                assert.ok(
                  output.value === 0n,
                  `Output ${i} value should be 0 if address is undefined`,
                );
              } else {
          ...
          it("should fail to parse with other wallet keys", function () {
            assert.throws(
              () => {
                bitgoPsbt.parseTransactionWithWalletKeys(createOtherWalletKeys(), {
                  replayProtection: { publicKeys: [replayProtectionKey] },
                });
              },
              (error: Error) => {
          ...
          it("should recognize output for other wallet keys", function () {
            const parsedOutputs = bitgoPsbt.parseOutputsWithWalletKeys(createOtherWalletKeys());
  ...
  describe("error handling", function () {
    it("should throw error for invalid PSBT bytes", function () {
      ...
      assert.throws(
        () => {
          fixedScriptWallet.BitGoPsbt.fromBytes(invalidBytes, "btc");
        },
        (error: Error) => {
