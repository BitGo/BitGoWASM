...
export function getFixtureInfo(network: MainnetCoinName): FixtureInfo {
  switch (network) {
    case "btc":
      return {
...
export function readFile(network: MainnetCoinName, path: string): string {
  const root = getArchiveRoot(getFixtureInfo(network));
  return readFileSync(`test/fixtures_thirdparty/nodes/${root}/src/test/data/${path}`, "utf8");
}

export function readJSON<T>(network: MainnetCoinName, path: string): T {
  return JSON.parse(readFile(network, path)) as T;
}
...
export function testFixture<T>(
  _ctx: Mocha.Suite,
  network: MainnetCoinName,
  filename: string,
  callback: (this: Mocha.Context, data: T) => void,
): void {
  it(filename, function () {
    callback.call(this, readJSON(network, filename));
  });
}
